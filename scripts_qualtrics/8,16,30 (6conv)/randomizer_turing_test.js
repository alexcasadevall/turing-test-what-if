Qualtrics.SurveyEngine.addOnload(function() {
    ...
    // 1. CONTROL DE REFRESCO: Si l'enquesta ja s'havia passat i hi ha dades al servidor, respecta-les
    ...
    // 2. ESCUT D'IFRAME PROTOCOLO MÒBIL: Protegim l'accés al pare perquè no faci caure el mòbil real
    let parentTuringDone = false;
    try {
        if (window.parent && window.parent !== window && window.parent.TURING_BRAIN_DONE) {
            parentTuringDone = true;
        }
    } catch (e) {
    ...
    if (parentTuringDone) {
        console.log("Cervell Turing detectat a la finestra pare. Copiant selecció única i omplint dades locals.");
        try {
    ...
    try {
        if (window.parent && window.parent !== window) {
            window.parent.TURING_FINAL = finalSelection;
            window.parent.TURING_BASE_URL = baseUrl;
            window.parent.TURING_BRAIN_DONE = true;
        }
    } catch (err) {}

    console.log("RANDOMISER DONE UNIQUE", finalSelection);
});